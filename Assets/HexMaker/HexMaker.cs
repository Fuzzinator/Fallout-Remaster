...
public class HexMaker : MonoBehaviour
{
    ...
    private void CreateNewCoords()
    {
        ...
        for (var z = 0; z < _horizontalCount; z++)
        {
            var zPos = z - (_centerOrig ? Mathf.Floor(_horizontalCount * .5f) : 0f);
            for (var x = 0; x < _verticalCount; x++)
            {
                Vector3 newPos;
                var xPos = x - (_centerOrig ? Mathf.Floor(_verticalCount * .5f) : 0f);
                if (_startTop)
                {
    ...
    private void CreateMeshAndCollider()
    {
        ...
        if (_createCollider)
        {
            var hasCollider = TryGetComponent(out _collider);
            if (hasCollider)
            {
